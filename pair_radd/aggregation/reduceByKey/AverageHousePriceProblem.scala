...
object AverageHousePriceProblem {
  ...
  def main(args: Array[String]) {
    ...
    Logger.getLogger("org").setLevel(Level.ERROR) // setting logging level to show only errors
    val conf = new SparkConf().setAppName("avgHousePrice").setMaster("local[3]")
    val sc = new SparkContext(conf)

    val lines = sc.textFile("in/RealEstate.csv")
    // remove all lines which does not contain Bedrooms
    val cleanedLines = lines.filter(line => !line.contains("Bedrooms"))

    // split by commas and get the fourth and second values int pair IDD (4th, (1, 2nd.toDouble))
    val housePricePairRdd = cleanedLines.map(line => (line.split(",")(3), (1, line.split(",")(2).toDouble)))

    // accessing pairRDD key and value using _1 and _2
    // get the summation of values (no of bedrooms and price) reduced by the Key(House)
    ...
    // using mapValues, calculate average considering values only
